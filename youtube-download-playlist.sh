...
function downloadChannelPlayList(){
	...
	-o "$DATADIR/Youtube/%(channel)s - %(channel_id)s/playlists/%(playlist)s - %(playlist_id)s/%(title)s - %(id)s/%(title)s - %(id)s.%(ext)s" \
	https://www.youtube.com/$channel/playlists
}

downloadChannelPlayList $1